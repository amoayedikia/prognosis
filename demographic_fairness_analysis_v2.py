...
class DemographicFairnessAnalyzer:
    ...
    def __init__(self, config: PROGRESSConfig, output_dir: str):
        ...
        self.total_subjects = 0  # Track total subjects
    ...
    def run_analysis(self, dataset: DemographicDataset,
        ...
        # Track total subjects
        self.total_subjects = len(dataset)
        ...
        for fold_idx, (train_idx, test_idx) in enumerate(skf.split(np.arange(len(dataset)), events)):
            ...
            # Log fold-level performance
            logger.info(f"  Fold {fold_idx + 1} results:")
            logger.info(f"    C-index: {fold_preds['metrics']['surv_c_index']:.4f}")
            logger.info(f"    α R²: {fold_preds['metrics']['traj_intercept_R2']:.4f}")
            logger.info(f"    Event rate: {fold_preds['metrics']['event_rate']:.3f}")
    ...
    def _train_and_predict(self, train_loader, val_loader, test_loader,
        ...
        traj_model, traj_epochs = self._train_model(
            traj_model, train_loader, val_loader,
            model_type='trajectory', num_epochs=num_epochs
        )
        
        # Train survival model
        surv_model, surv_epochs = self._train_model(
            surv_model, train_loader, val_loader,
            model_type='survival', num_epochs=num_epochs
        )
        
        logger.info(f"    Training completed - Traj: {traj_epochs} epochs, Surv: {surv_epochs} epochs")
    ...
    def _train_model(self, model, train_loader, val_loader,
                     model_type: str, num_epochs: int) -> Tuple[nn.Module, int]:
        """Train a single model with early stopping. Returns (model, epochs_trained)."""
        ...
        return model, final_epoch  # Return epoch count for debugging
    ...
    def generate_summary_table(self) -> pd.DataFrame:
        ...
        # Overall row - use actual total subjects
        rows.append({
            'Subgroup': 'Overall',
            'N': self.total_subjects,  # Correct count
